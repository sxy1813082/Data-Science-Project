...
# def sample_predictive_model(model, guide, num_samples, *args):
#     predictive = Predictive(model, guide=guide, num_samples=num_samples)
#     sampled_models = predictive(*args)
#     # take mean over all sampled models for prediction
#     predictions = sampled_models['obs'].mean(0).argmax(dim=-1).cpu().numpy()
#     return predictions
def sample_predictive_model(model, guide, num_samples, inputs, labels=None):
...
def bald_sampling(model, k, num):
    ...
    with torch.no_grad():
        ...
        # Calculate the BALD scores for each embedding
        bald_scores = []
        for i, embedding in enumerate(embeddings):
            # Sample from the model multiple times and calculate the predictive entropy
            num_samples = 9  # can experiment with different numbers of samples
            entropy = 0
            for _ in range(num_samples):
                sampled_logits = model(embedding.unsqueeze(0))  # Add a batch dimension
                sampled_probs = torch.softmax(sampled_logits, dim=-1)
                entropy += torch.sum(-sampled_probs * torch.log(sampled_probs))
            avg_entropy = entropy / num_samples
            bald_score = -avg_entropy.item()  # Negate the entropy for maximization
            bald_scores.append(bald_score)

        # Select the top k samples with the highest BALD scores
        selected_indices = np.argsort(bald_scores)[-k:].tolist()
